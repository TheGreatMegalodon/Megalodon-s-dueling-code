...
const shipCodes = [101, 201, 202, 301, 302, 303, 304, 401, 402, 403, 404, 405, 406, 501, 502, 503, 504, 505, 506, 507, 601, 602, 603, 604, 605, 606, 608, 701, 702, 703, 704];
const ships = [
...
this.tick = function(game) {
	game.ships.forEach(ship => {
		if (game.step % 30 === 0) {
			...
			if (ship.crystals >= maxCrystals) ship.set({
				crystals: maxCrystals - 1
			});
		}
	});
};
	...
	switch (event.name) {
		...
		case "ship_spawned":
			...
			if (event.ship.custom.init !== true) {
				...
				event.ship.setUIComponent(switch_button);
...
function switchShip(ship) {
	...
	if (ship.type === 192) {
		...
		ship.set({
			...
			shield: 999,
			stats: max_stats,
			crystals: 20 * Math.trunc(next_type / 100) ** 2
		});
	}
	ship.set({
		vx: 0,
...
set = function(who, what, max_crystals, max_stats) {
	if (!max_stats && max_stats !== 0)
		if (Math.trunc(game.ships[who].type / 100) < 7) max_stats = 11111111 * Math.trunc(game.ships[who].type / 100);
		else max_stats = 0;
	else max_stats;
	if (!max_crystals) max_crystals = 20 * Math.trunc(game.ships[who].type / 100) * Math.trunc(game.ships[who].type / 100);
	else max_crystals;
	game.ships[who].set({
		type: what,
		crystals: max_crystals,
...
admin = function(who) {
	...
	if (game.ships[who].type === 192) {
		if (Math.trunc(game.ships[who].custom.oldAdminShip / 100) < 7) max_stats = 11111111 * Math.trunc(game.ships[who].custom.oldAdminShip / 100);
		else max_stats = 0;
		...
		game.ships[who].set({
			...
			stats: max_stats,
			invulnerable: 240,
			type: game.ships[who].custom.oldAdminShip
		});
	} else {
